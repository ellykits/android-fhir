...
function build_only() {
  ...
  ./gradlew build -Pcatalog.wasm.enabled=false -x :datacapture-kmp:build -x :sdc-kmp-demo:build --scan --stacktrace
  ./gradlew check -Pcatalog.wasm.enabled=false -x :datacapture-kmp:check -x :sdc-kmp-demo:check --scan --stacktrace
}
